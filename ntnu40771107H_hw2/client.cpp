    ...
    while( getline(cin,instruction) ){
        ...
        //=====PUT FILE FROM SERVER====================
            ...
            */
            ...
            sprintf(buffer,"%d",amount_of_file);
            if((write_byte = send(sockfd, buffer, strlen(buffer), 0)) < 0){
                    ERR_EXIT("write failed\n");
                    return -1;
            }
            // Check server get message
           ...
           // printf("%s\n",buffer);
            ...
            for( string file : upload_files ){
                ...
                if( it != my_files.end() ){
                    ...
                    sprintf(buffer,"%s %lld%c",file.c_str(),file_size,'\0');
                    //puts(buffer);
                    if((write_byte = send(sockfd, buffer, strlen(buffer), 0)) < 0){
                        ERR_EXIT("write failed\n");
                        return -1;
                    ...
                    while( file_size > 0  ){
                        ...
                        new_file.read(content,sizeof(char)*tmp_size);
                        //printf("%ld\n",content.size());
                        if((write_byte = send(sockfd, content, tmp_size, 0)) < 0){
                            ERR_EXIT("write failed\n");
                            return -1;
                        }
                        free(content);
        ...
        //=====GET FILE FROM SERVER=================
            ...
            if((read_byte = read(sockfd, buffer, 3)) < 0){
                ERR_EXIT("receive failed\n");
                return -1;
            }
            //Sending amount_of_file to server
            ...
            for( string file:upload_files ){
                ...
                if( file_size > 0 ){
                    ...
                    sprintf(buffer,"%s","OK");
                    //Send message to server ready to get hole file
                    if((write_byte = send(sockfd,buffer , strlen(buffer), 0 )) < 0){
                        ERR_EXIT("write failed\n");
                        return -1;
                    }
                    ...
                    while( file_size > 0  ){
                        ...
                        get_size( tmp_size,file_size );
                        char *content = new char[BUFF_SIZE]();
                        if((read_byte = read(sockfd, content, sizeof(char)*tmp_size ))< 0){
                            ERR_EXIT("receive failed\n");
                            return -1;
                        }
                        //===Write in file===
                        ...
                        new_file.write( content,tmp_size );
                        sprintf(buffer,"%s","OK");
                        //Send message to client ready to get file content
                        if((write_byte = send(sockfd,buffer , strlen(buffer), 0 )) < 0){
                            ERR_EXIT("write failed\n");
                            return -1;
                        }
